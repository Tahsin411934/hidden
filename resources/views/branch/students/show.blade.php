...
<div class="max-w-7xl mx-auto bg-white rounded-xl shadow-2xl overflow-hidden border border-gray-100">
    ...
    <div class="bg-gradient-to-r from-[#006172] to-[#0096a0] py-4 px-8 text-white">
        <div class="flex justify-between items-center">
            ...
            <span class="bg-white/20 px-3 py-1 rounded-full text-sm font-medium">{{ $students->registration_no ?? 'NA' }}</span>
        </div>
        <div class="flex items-center mt-2 text-white/90">
            <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 mr-1" viewBox="0 0 20 20" fill="currentColor">
                <path fill-rule="evenodd" d="M6 2a1 1 0 00-1 1v1H4a2 2 0 00-2 2v10a2 2 0 002 2h12a2 2 0 002-2V6a2 2 0 00-2-2h-1V3a1 1 0 10-2 0v1H7V3a1 1 0 00-1-1zm0 5a1 1 0 000 2h8a1 1 0 100-2H6z" clip-rule="evenodd" />
            </svg>
            <span class="text-sm">Last updated: {{ $students->updated_at->format('M d, Y') ?? 'NA' }}</span>
        </div>
    </div>
            ...
            <div class="lg:col-span-1 space-y-6">
                ...
                <div class="bg-gray-50 p-5 rounded-xl border border-gray-200 shadow-sm">
                    <div class="flex flex-col items-center">
                        ...
                        <h3 class="text-xl font-bold text-gray-800">{{ $students->name ?? 'NA' }}</h3>
                        <p class="text-[#006172] font-medium">{{ $students->course->name ?? 'NA' }}</p>
                    </div>
                </div>

                <!-- Academic Details Card (Always Readonly) -->
                <div class="bg-gray-50 p-5 rounded-xl border border-gray-200 shadow-sm">
                    ...
                    <div class="space-y-3">
                        <div>
                            ...
                            <input type="text" value="{{ $students->roll_no ?? 'NA' }}" 
                                   class="w-full p-2 bg-gray-100 rounded-md border-none" readonly disabled>
                        </div>
                        <div>
                            <p class="text-xs font-medium text-gray-500">Session</p>
                            <input type="text" value="{{ $students->session ?? 'NA' }}" 
                                   class="w-full p-2 bg-gray-100 rounded-md border-none" readonly disabled>
                        </div>
                        <div>
                            <p class="text-xs font-medium text-gray-500">Status</p>
                            <div class="flex items-center">
                                <input type="text" value="{{ $students->status ?? 'NA' }}" 
                                       class="w-full p-2 bg-gray-100 rounded-md border-none" readonly disabled>
                        ...
                        <div>
                            <p class="text-xs font-medium text-gray-500">Registration No</p>
                            <input type="text" value="{{ $students->registration_no ?? 'NA' }}" 
                                   class="w-full p-2 bg-gray-100 rounded-md border-none" readonly disabled>
                        </div>
                        <div>
                            <p class="text-xs font-medium text-gray-500">Branch Code</p>
                            <input type="text" value="{{ $students->branc_code ?? 'NA' }}" 
                                   class="w-full p-2 bg-gray-100 rounded-md border-none" readonly disabled>
                        </div>
                    </div>
                </div>
                ...
                <div class="bg-white p-6 rounded-xl border border-gray-200 shadow-sm">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            ...
                            <input type="text" name="name" value="{{ $students->name ?? 'NA' }}" 
                        ...
                        <div>
                            ...
                            <input type="text" name="father_name" value="{{ $students->father_name ?? 'NA' }}" 
                                   class="w-full p-2 rounded-md border border-gray-300 focus:ring-2 focus:ring-[#006172]/50 focus:border-[#006172] read-only:bg-gray-50" readonly>
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-1">Mother's Name</label>
                            <input type="text" name="mother_name" value="{{ $students->mother_name ?? 'NA' }}" 
                                   class="w-full p-2 rounded-md border border-gray-300 focus:ring-2 focus:ring-[#006172]/50 focus:border-[#006172] read-only:bg-gray-50" readonly>
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-1">Phone Number</label>
                            <input type="text" name="phone_number" value="{{ $students->phone_number ?? 'NA' }}" 
                                   class="w-full p-2 rounded-md border border-gray-300 focus:ring-2 focus:ring-[#006172]/50 focus:border-[#006172] read-only:bg-gray-50" readonly>
                        </div>
                        <div class="md:col-span-2">
                            <label class="block text-sm font-medium text-gray-700 mb-1">Address</label>
                            <input type="text" name="address" value="{{ $students->address ?? 'NA' }}" 
                                   class="w-full p-2 rounded-md border border-gray-300 focus:ring-2 focus:ring-[#006172]/50 focus:border-[#006172] read-only:bg-gray-50" readonly>
                        </div>
                    </div>
                </div>
                ...
                <div class="bg-white p-6 rounded-xl border border-gray-200 shadow-sm">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            ...
                            <input type="text" name="tana" value="{{ $students->tana ?? 'NA' }}" 
                                   class="w-full p-2 rounded-md border border-gray-300 focus:ring-2 focus:ring-[#006172]/50 focus:border-[#006172] read-only:bg-gray-50" readonly>
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-1">Village</label>
                            <input type="text" name="vill" value="{{ $students->vill ?? 'NA' }}" 
                                   class="w-full p-2 rounded-md border border-gray-300 focus:ring-2 focus:ring-[#006172]/50 focus:border-[#006172] read-only:bg-gray-50" readonly>
                        </div>
                    </div>
                </div>
...
<script>
    ...
    const formInputs = document.querySelectorAll('#studentForm input:not([type="file"]):not([disabled])');
    ...
    editBtn.addEventListener('click', () => {
        // Enable all editable text inputs
