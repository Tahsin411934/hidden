<x-guest-layout>
    <div class="bg-gradient-to-r from-[#003E59] to-[#28708f] px-36 py-4">
        <h2 class="text-3xl font-bold text-[#f7f9fa]">Student Result</h2>
        <p class="text-base text-[#f7f9fa] mt-1">Search for student results by roll and registration number.</p>
    </div>
    <div class="min-h-screen max-w-4xl mx-auto bg-gray-50 py-8 px-4 sm:px-6 lg:px-8">
        <div class=" mx-auto">
            <!-- Header -->
            ...
            <div class="bg-white shadow rounded-lg overflow-hidden mb-8">
                ...
                <div class="px-6 py-5">
                    <form method="POST" action="{{ route('student-result') }}" class="space-y-6">
                        @csrf
                    ...
                    <div class="grid grid-cols-1 gap-8 md:grid-cols-2">
                        ...
                        <div>
                            ...
                            <div class="space-y-3">
                                ...
                                <div class="flex items-start">
                                    <span class="w-32 flex-shrink-0 text-sm font-medium text-gray-500">Father's
                                        Name</span>
                                    <span class="text-sm text-gray-900">{{ $student->father_name ?? 'N/A' }}</span>
                                </div>
                                <div class="flex items-start">
                                    <span class="w-32 flex-shrink-0 text-sm font-medium text-gray-500">Mother's
                                        Name</span>
                        ...
                        <div>
                            ...
                            <div class="space-y-3">
                                ...
                                <div class="flex items-start">
                                    ...
                                    <span
                                        class="text-sm text-gray-900">{{ $student->branch->branch_name ?? 'N/A' }}</span>
                                </div>
                                <div class="flex items-start">
                                ...
                                <tbody class="bg-white divide-y divide-gray-200">
                                    ...
                                    <tr>
                                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                            {{ $mark->academic_assessment ?? 'N/A' }}</td>
                                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                            {{ $mark->written ?? 'N/A' }}</td>
                                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                            {{ $mark->practical ?? 'N/A' }}</td>
                                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                            {{ $mark->viva ?? 'N/A' }}</td>
                                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                            {{ $mark->total_obtain_marks ?? 'N/A' }}</td>
                                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                            {{ $mark->grade ?? 'N/A' }}</td>
                                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                            {{ $mark->gpa ?? 'N/A' }}</td>
